...
- удаление загруженного аудио из Files API после распознавания;
...
- выбор локального аудиофайла через диалог или drag-and-drop;
- загрузка аудиодорожки по ссылке на YouTube, Rutube, VK и другие сайты, поддерживаемые `yt-dlp`;
- переключение источника двумя чекбоксами «Локальный файл» / «Ссылка»;
- режимы `verbatim` и `smart`;
- языки в формате BCP-47;
- разделение по говорящим и словесные таймстемпы;
- custom vocabulary;
- сохранение TXT и JSON;
- выполнение скачивания и транскрибации в отдельном потоке, поэтому интерфейс остаётся отзывчивым;
- остановка текущей операции кнопкой «Остановить» и прогресс загрузки по ссылке;
- понятные сообщения для сетевых ошибок, тайм-аутов, проблем API-ключа, квоты, cookies, Node.js и FFmpeg.

В режиме «Ссылка» вставьте URL видео или аудио в поле источника. `yt-dlp` скачает только лучшую доступную аудиодорожку и приведёт её к формату `m4a`, после чего файл будет передан Gemini для транскрибации. Плейлисты не скачиваются: обрабатывается только указанный ролик.

После нажатия «Остановить» приложение прерывает загрузку через `yt-dlp` или активный асинхронный запрос к Gemini, не сохраняет и не показывает отменённый результат, затем очищает временные файлы.

Если YouTube показывает ошибку `Sign in to confirm you’re not a bot`, экспортируйте cookies из браузера в формате Netscape и укажите файл в `.env`:

```env
YTDLP_COOKIES_FILE=cookies.txt
```

Положите `cookies.txt` рядом с `.env` либо укажите абсолютный путь. Файл cookies содержит данные сессии: не публикуйте его и не добавляйте в Git. Без этой настройки остальные сайты, поддерживаемые `yt-dlp`, работают как обычно.
...
Требуется Python 3.10 или новее. Для скачивания и конвертации аудиодорожек по ссылкам также нужен `FFmpeg`, доступный в `PATH`. Проверить его наличие можно командой `ffmpeg -version`.

Для загрузки с YouTube нужен поддерживаемый JavaScript runtime. В проекте используется `Node.js`, доступный в `PATH`, и пакет `yt-dlp-ejs`, устанавливаемый из `requirements.txt`. Проверить Node.js можно командой `node --version`. Поддерживаемый yt-dlp `Deno` также остаётся включённым, если он установлен.
...
Включает разделение текста по говорящим, например `spk_1` и `spk_2`.
...
Запрашивает таймстемпы каждого слова. При включённых `--speaker-diarization` или `--timestamps` TXT-файл содержит сегменты с говорящими и/или временными интервалами: текст берётся из ответа модели (с её пунктуацией), сегменты разделяются по смене говорящего и по границам предложений. Без этих параметров TXT содержит сплошной текст расшифровки. `--json-output` дополнительно сохраняет исходные аннотации ответа API:
...
Юнит-тесты покрывают форматирование расшифровки, сообщения об ошибках, определение пути скачанного аудио и запись результатов. Сеть и API-ключ не нужны:
